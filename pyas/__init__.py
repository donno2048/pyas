...
def function(asm: str, /, *args, raw = True, func_name = "main"):
	...
	if platform() == "Linux":
		...
		wasmTable.set(ptr, eval(f"(_) => _.exports.{func_name}")(WebAssembly.Instance.new(WebAssembly.Module.new(to_js((str().join(chunk.strip() for l in asm.splitlines() for chunk in l.split())).encode())))))
